...
/** Smallest angle between two line orientations (π-periodic), in radians. */
...
/** Interior angle (degrees) at `curr` formed by the segments curr→prev and curr→next. */
export function cornerAngleDeg(prev: Point, curr: Point, next: Point): number {
...
/** Largest deviation (degrees) of any corner angle from 90°. */
export function maxCornerAngleDeviation(corners: Quad): number {
  let maxDeviation = 0;
  for (let i = 0; i < 4; i++) {
    const prev = corners[(i + 3) % 4];
    const curr = corners[i];
    const next = corners[(i + 1) % 4];
    const deviation = Math.abs(cornerAngleDeg(prev, curr, next) - 90);
